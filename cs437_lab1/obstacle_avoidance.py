...
OBSTACLE_SCAN_POSITIONS = [FAR_LEFT_SERVO_POSITION, FAR_RIGHT_SERVO_POSITION]
...
def avoid_obstacle():
    ...
    if not valid_positions:
        ...
        print(
            "NO VALID DIRECTIONS FOUND DURING INITIAL SWEEP, BACKING UP AND RESCANNING"
        )
        reverse()
    ...
    if valid_positions:
        print(
            f"VALID DIRECTIONS FOUND DURING SWEEP: {[x.name for x in valid_positions]}"
        )
...
def process_historical_center_readings():
    ...
    if len(CENTER_DISTANCE_HISTORY) == CENTER_DISTANCE_SAMPLE_COUNT:
        ...
        delta_distance_change = max(CENTER_DISTANCE_HISTORY) - min(
            CENTER_DISTANCE_HISTORY
        )
